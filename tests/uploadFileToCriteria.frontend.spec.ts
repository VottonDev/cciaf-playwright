...
const DOCUMENT_TITLE = `sample_${Date.now()}`;
const DOCUMENT_PATH = path.join(__dirname, '../upload_samples/sample.pdf');

async function navigateToDocumentLibrary(page) {
...
test('Step 1: Navigate to criteria and verify upload form', async ({ page }) => {
  ...
  await page.waitForLoadState('networkidle');
  await axeScan(page, 'practice-area-overview', { 
    softFail: true,
  ...
  await page.waitForLoadState('networkidle');
  await axeScan(page, 'criteria-detail-before-upload', { 
    softFail: true,
    include: ['main', '.govuk-main-wrapper'],
    exclude: ['#navigation', 'header', '.govuk-header']
  });
  ...
  await page.waitForLoadState('networkidle');
  await axeScan(page, 'document-upload-form', { 
    softFail: true,
    include: ['main', '.govuk-main-wrapper'],
    exclude: ['#navigation', 'header', '.govuk-header']
  });
...
test('Step 3: Navigate to document library and verify unpublished state', async ({ page }) => {
  ...
  await axeScan(page, 'document-library-before-publish', { 
    softFail: true,
    include: ['main', '.govuk-main-wrapper'],
    exclude: ['#navigation', 'header', '.govuk-header']
  });
...
test('Step 4: Publish the document', async ({ page }) => {
  ...
  await page.waitForLoadState('networkidle');
  await expect(documentRow.getByText('Published')).toBeVisible();
...
test('Step 5: Verify document is published on criteria page', async ({ page }) => {
  ...
  await page.waitForLoadState('networkidle');
  await axeScan(page, 'criteria-detail-with-published-document', { 
    softFail: true,
    include: ['main', '.govuk-main-wrapper'],
    exclude: ['#navigation', 'header', '.govuk-header']
  });
...
test('Step 7: Verify document is removed from criteria', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'Criteria 1.1.1' }).click();
  await page.waitForLoadState('networkidle');
  await axeScan(page, 'criteria-detail-after-remove', { 
    softFail: true,
    include: ['main', '.govuk-main-wrapper'],
    exclude: ['#navigation', 'header', '.govuk-header']
  });
